...
async function queueImplementation(stepId) {
    ...
    const prompt = [
        ...
        "All work — runtime/EE/JIT contract changes and collector implementation alike — belongs in the kkokosa/runtime project (the runtime-fork project), on a new branch off the project default. The abandoned C:\\github\\runtimelab prototype is not a source of code, status, or measurements; do not read from it or branch from it.",
        ...
        "P0.1 established facts that bind later steps: the PLDI oracle fails under sustained load, tripping its own barrier assertion `assert!(old.is_null() || rc::count(old) != 0, \"zero rc count\")` at src/plan/barriers.rs:315 in FieldLoggingBarrier::slow, with the cfg(any(sanity, debug_assertions)) gate explaining why release SIGSEGVs on the same corrupt state; the defect is load-dependent rather than revision-specific. So RC and SATB mechanisms follow specify-from-PLDI, validate-against-lxr-head. RC width is a designed tunable (LOG_REF_COUNT_BITS selectable via lxr_rc_bits_{2,4,8}, default 2 bits with MAX_REF_COUNT == 3) and a port must parameterize rather than hardcode it. At the PLDI oracle there is no src/plan/lxr/ tree at all: LXR is a compile-time feature configuration of the Immix plan, so cross-revision comparison is mechanism-to-mechanism and never path-to-path.",
        "P0.1 artifacts live in docs/design/lxr-port/ — read them from main if present there (merged via kkokosa/runtime PR #1), otherwise from branch kkokosa-p01-pin-reference-revisions at commit 66991f19, e.g. `git show kkokosa-p01-pin-reference-revisions:docs/design/lxr-port/P0.1-mechanism-diff.md`. Do not branch off that branch unless this step's Dependencies say otherwise. Its scripts/ record how the reference was built and reference the WSL build root /root/lxr and the reference checkout C:\\github\\lxr-reference, which are environment paths rather than repository paths.",
