...
async function queueImplementation(stepId) {
    ...
    const prompt = [
        ...
        "P0.1 and P0.2 artifacts live in docs/design/lxr-port/ on main, merged via kkokosa/runtime PRs #1 and #2 — read them from main, e.g. `git show main:docs/design/lxr-port/P0.1-mechanism-diff.md` and `P0.2-paper-targets.md`. Its scripts/ record how the reference was built and reference the WSL build root /root/lxr and the reference checkout C:\\github\\lxr-reference, which are environment paths rather than repository paths.",
