...
50. **Re-verifying against a stale ref is the most persuasive way to be wrong, because the evidence is real.** The P0.5 session reported two defects "re-verified just now, both still live" against `e68c3daa489` — which is the direct parent of `3cadda3d54f`, its own commit fixing them. I checked all three trees: at `e68c3daa489` both are present, so its finding was *true of the tree it examined* and false of the tree that exists. This is worse than an unevidenced stale claim, because the evidence survives scrutiny and only the target is wrong. **"Verified just now" fixes when the command ran, not how old the thing it ran against is** — recency of check and recency of target are independent, and only the first is visible in the report. When asking whether a defect is still live, resolve the ref at the moment of use and check the head, not the hash last read.
51. **When a child re-sends materially the same message after a correcting reply, the reply is what is producing it.** Three near-identical messages arrived, each opening by explaining the crossing, each re-reporting defects already fixed. Answering a fourth time extends the loop rather than closing it, because a reply is a turn and a turn produces a message. Close with an explicit instruction not to reply, state what would justify breaking silence, and stop — a coordinator who cannot stop talking cannot tell a child to stop. Worth noting against rule 41: I earlier recorded the reply-loop theory as *disproved* in favour of concurrent rewriting. With no concurrent rewriting in flight this round, the loop recurred, so the original theory was right and my correction of it was wrong. A hypothesis discarded because a second cause was found is not thereby refuted.
