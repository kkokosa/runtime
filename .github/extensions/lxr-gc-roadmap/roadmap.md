...
- **Evidence:** PR #5 at `1345c1b1b29dea2c4965cdb15f397717308fcfdc`, 57 files, all under `docs/design/lxr-port/`. Verified from a `git archive` extract outside any worktree: gate `PASS (34 checks)` exit 0 with one anchored `^RESULT: PASS` line, `--no-results` → 3, unknown flag → 2, checkpoints removed → 2, with zero anchored PASS lines in each refusal counted rather than eyeballed. The perturbation set ships as a runnable script rather than a table of results — `verify-gate-of-the-gate.sh` reports `PASS (11 of 11 cases)` and now derives its own closing summary, "passed 2 clean extract(s), refused 1, and failed 8 single perturbation(s)", from the exit code each case requires. `check-prose-claims.py` re-derives 12 quantified prose claims from the shipped CSVs at run time, none stored as a constant; eight line-targeted perturbations of §5.1 each exit 1 with zero anchored PASS lines, covering the binding count, Workstation attainment, ties, the distinct-value count, a substituted value, an **added** value, the smallest-value scenario count and the server range — the added-value case having passed at two earlier hashes, which is rule 36. 58 unit tests pass built from the committed tree rather than from the author's binaries, against a harness tree hash unchanged since that run. 1,771 evidence files (19.65 MB) preserved from 15.2 GB of run artifacts before archival, SHA-256 checked against a negative control.
