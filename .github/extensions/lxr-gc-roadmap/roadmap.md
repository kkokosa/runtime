...
46. A battery needs an assertion at **both** ends, because observing only one verdict cannot distinguish a working check from a broken battery. Observing only FAIL cannot tell "the check fired" from "the harness never reached the document"; observing only PASS cannot tell "no defect" from "the perturbation did not apply". The implementer hit the first end for real: invoking the prose checker with `--results` pointing one directory too deep produced **exit 1 with five `FAIL missing input` lines on a completely unperturbed tree**. A battery that scored FAIL as success would have reported four checks firing while the checker had never read the document. I reproduced it rather than take it, and found the discriminator the report did not name: **the broken run emits `exit≠0` and ZERO anchored `^RESULT:` lines, while a genuinely fired check emits `exit≠0` and exactly ONE `^RESULT: FAIL`**. So the remedy is mechanical and needs no extra scaffolding — assert on the anchored verdict line rather than on the exit code, which is rule 12's anchoring discipline arriving at the same place from the opposite direction. Keep the pristine control as well, since it catches the class where the harness reads the document and still misjudges it. The other end has its own guard: compare the file before and after substitution and abort if unchanged, which is what saves the reorder and remove cases from silently passing.
