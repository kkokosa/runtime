...
- **Benchmarks:** `wks` and `srv` only, verified through the live canvas — 3 checkpoints, 144 collector fields, 0 naming LXR, 0 load problems. Within-session `srv`/`wks` ratio drift is 7.28% median against 19.06% for absolutes, with all four throughput ratios inside the floor, so **P0.6 onward must measure LXR against a baseline arm in the same session and treat the ratio as the quantity of record**; absolute throughput here carries ±28% session-to-session, several times anything the paper reports. Throughput CV is 1.60% median with 0 of 59 cells above the floor, while p99 latency CV is 11.67% median with 34 of 60 above it — the quantity LXR is meant to win on is the one this host resolves worst, which sets a hard limit on what any later latency claim can assert.
