...
if (-not $OutputDirectory) {
    $OutputDirectory = Join-Path $RepositoryRoot (
        "artifacts\p15-reference-enumeration-validation\$Architecture")
}

$gcRoot = Join-Path $RepositoryRoot 'src\coreclr\gc'
$source = Join-Path $scriptRoot 'reference-enumeration-validation.cpp'
$executable = Join-Path $OutputDirectory 'reference-enumeration-validation.exe'
$object = Join-Path $OutputDirectory 'reference-enumeration-validation.obj'
