...
output_root="${2:-$repository_root/artifacts/P2.1/validation/linux-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
...
command=(
  ...
  -I"$native_root"
  -I"$native_root/inc"
  "$script_root/side-metadata-validation.cpp"
  "$script_root/side-metadata-test-platform.cpp"
  "$gc_root/side_metadata.cpp"
  -o "$output_root/side-metadata-validation"
)

printf '%q ' "${command[@]}" > "$output_root/command.txt"
printf '\n' >> "$output_root/command.txt"
"${command[@]}" > "$output_root/build.log" 2>&1
"$output_root/side-metadata-validation" > "$output_root/run.log" 2>&1
result="$(sed -nE 's/^([0-9]+)\/([0-9]+) side metadata checks passed$/\1 \2/p' "$output_root/run.log")"
...
sed -nE 's/^([0-9]+)\/([0-9]+) side metadata checks passed$/linux-x64,\1,\2,PASS/p' \
  "$output_root/run.log" >> "$output_root/validation-summary.csv"
printf 'PASS: Linux x64 side-metadata validation\n'
printf 'Output: %s\n' "$output_root"
