...
if (-not $OutputDirectory) {
    $OutputDirectory = Join-Path $RepositoryRoot (
        'artifacts\P2.2\validation\' + [guid]::NewGuid().ToString('N'))
} else {
...
$validation = Join-Path $scriptRoot 'immix-block-validation.cpp'
$platform = Join-Path $scriptRoot '..\P2.1\side-metadata-test-platform.cpp'
$sideMetadata = Join-Path $gcRoot 'side_metadata.cpp'
$immixBlock = Join-Path $gcRoot 'immix_block.cpp'
...
foreach ($inputPath in $requiredInputs) {
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required validation input is missing: $inputPath"
    }
}
...
foreach ($architecture in @('x64', 'x86')) {
    ...
    $executable = Join-Path $target 'immix-block-validation.exe'
    ...
    $command = @(
        ...
        "cd /d `"$target`"",
        "cl /nologo /std:c++17 /EHsc /W4 /WX /wd4100 /wd4324 /DBUILD_AS_STANDALONE /DTARGET_WINDOWS /DHOST_WINDOWS /DWIN32 $defines /I`"$gcRoot`" /I`"$gcRoot\env`" /I`"$nativeRoot`" /I`"$nativeRoot\inc`" `"$validation`" `"$platform`" `"$sideMetadata`" `"$immixBlock`" /Fe:`"$executable`"",
        "`"$executable`""
    ) -join ' && '
    ...
    if ($exitCode -ne 0) {
        throw "Immix block validation failed for $architecture. See $log."
    }

    $match = Select-String -LiteralPath $log -Pattern '(\d+)/(\d+) immix block checks passed'
    if ($match.Count -ne 1) {
        throw "Immix block validation result is missing for $architecture."
    }
    $passed = [int]$match.Matches[0].Groups[1].Value
    $total = [int]$match.Matches[0].Groups[2].Value
    if (($passed -ne $total) -or ($total -le 0)) {
        throw "Immix block validation was incomplete for $architecture."
    }
...
Write-Host "PASS: $($summary.Count) Windows Immix block validation targets"
